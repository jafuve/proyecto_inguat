...
namespace Proyecto_Inguat
{
    class AlgoritmoDijkstra
    {
        ...
        public int[,] setGrafo()
        {
            ...
            matrizTemporal = tamanioMatriz(matrizTemporal, Directory.GetCurrentDirectory() + GlobalVariables.DB_Route_File);

            System.IO.StreamReader file =
                new System.IO.StreamReader(Directory.GetCurrentDirectory() + GlobalVariables.DB_Route_File);
            ...
            while ((linea = file.ReadLine()) != null)
            {
                ...
                // 0 = ID, 1 = FROM ID, 2 = FROM NAME, 3 = TO ID, 4 = TO NAME, 5 DISTANCE, 6 ACTIVE

                int fromIdx = 1, toIdx = 3, distanceIdx = 5;

                bool canConvert1 = int.TryParse((split[0].ToString()), out valTemp);
                if (canConvert1 != true)
                    matrizTemporal[cont, 0] = Convert.ToChar(split[fromIdx].ToString()) - 65;
                else
                    matrizTemporal[cont, 0] = Convert.ToInt32(split[fromIdx].ToString());//Si es un numero, directamente se almacena en la matriz temporal

                bool canConvert2 = int.TryParse((split[1].ToString()), out valTemp);
                if (canConvert2 != true)
                    matrizTemporal[cont, 1] = Convert.ToChar(split[toIdx].ToString()) - 65;
                else
                    matrizTemporal[cont, 1] = Convert.ToInt32(split[toIdx].ToString());


                matrizTemporal[cont, 2] = Convert.ToInt32(split[distanceIdx].ToString());
                cont++;
            }
            cont = 0;
            file.Close();
            ...
            int nNodos = 0;
            //NUMERO DE NODOS SE OBTIENE DEL ARCHIVO PLACES
            //[EDITED]
            ...
            else
            {
                ...
                nNodos = System.IO.File.ReadAllLines(path).Length;
            }//END IF
            //[END EDITED]
        ...
        public int[,] tamanioMatriz(int[,] matrizTemporal, string nombre)
        {
            ...
            System.IO.StreamReader file =
                new System.IO.StreamReader(nombre); //Linea que sirve para leer el archivo de texto
        ...
        public string imprimir() { return imprimirRecorrido(matrizRecorrido); }

        public string imprimirRecorrido(int[,] matriz)
        {
